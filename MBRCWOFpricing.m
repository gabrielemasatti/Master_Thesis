function [Price] = MBRCWOFpricing(Nsim, ULsMktData, SetDate, Ncoupons, CouponFreq, CouponBarrier, CouponLevel, FinalBarrier, MarginalParams, CommonParameters)
%
...
A2Fwd = A2S0*exp((A2MonRates(1) - A2MonDiv(1))*yearfrac(SetDate, MonDates(2), 3));
...
%% simulations and Digital Payout as in chapter 4.3 S-IG
for i = 1:Ncoupons

    t = yearfrac(SetDate, MonDates(i+1), 3);
    s = yearfrac(SetDate, MonDates(i), 3);
    
    [f1(:,i), f2(:,i)] = simulateLogFwd(Nsim, 14, 1, t, s, MarginalParams, CommonParameters);
...
%     if (i~=Ncoupons)
%         DiscPayout(:, i) = A1MonDisc(i).*DigPayout(:, i);
%     end
