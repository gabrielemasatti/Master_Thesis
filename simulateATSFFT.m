function [X] = simulateATSFFT(M, Nsim, flagspline, t, s,sigma_t,sigma_s,eta_t,eta_s,k_t,k_s, U, q)
%
%
...
alpha       = 0.5;
IBDaycount  = 3;
b           = (1-alpha)^(1-alpha)./(2^(alpha)*alpha).*(t^q./k_t.^(1-alpha).*sigma_t.^(2*alpha) - s^q./k_s.^(1-alpha).*sigma_s.^(2*alpha)) - 1e-4;
omega       = 2*alpha-1e-4;
...
charFun_t = @(u) exp(1/k_t *(1 - sqrt(1 + u.^2.*t^(2*q)*sigma_t^2*k_t- 2i.*eta_t.*t^q.*u*k_t)));
c_t       = -log(charFun_t(-1i));
charFun_s = @(u) exp(1/k_s *(1 - sqrt(1 + u.^2.*s^(2*q)*sigma_s^2*k_s- 2i.*eta_s.*s^q.*u*k_s)));
c_s       = -log(charFun_s(-1i));

% charFun_t = @(u) exp(1/k_t *(1 - sqrt(1 + u.^2.*t^(2*q)*sigma_t^2*k_t- 2i.*eta_t.*t^q.*u*k_t)));
% charFun_s = @(u) exp(1/k_s *(1 - sqrt(1 + u.^2.*s^(2*q)*sigma_s^2*k_s- 2i.*eta_s.*s^q.*u*k_s)));
% phi_s_t   = @(u) (exp(1i.*u.*c_t).*charFun_t(u))./(exp(1i.*u.*c_s).*charFun_s(u));

% charFun_t = @(u) exp(1/k_t *(1 - sqrt(1 + u.^2.*t^(2*q)*sigma_t^2*k_t- 2i.*eta_t.*t^q.*u*k_t)));
% charFun_s = @(u) exp(1/k_s *(1 - sqrt(1 + u.^2.*s^(2*q)*sigma_s^2*k_s- 2i.*eta_s.*s^q.*u*k_s)));
phi_s_t   = @(u) charFun_t(u)./charFun_s(u);
