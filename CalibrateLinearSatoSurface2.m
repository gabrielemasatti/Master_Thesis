function [MarginalParams, CommonParams, CalibError, CalibCorr] = CalibrateLinearSatoSurface2(SetDate, ...
...
for i = 1:length(A1TTM)
    ...
    if (i==1)
        ...
        A1MdlPricesCall     = @(cal) CallPricesLinearSatoFFT2(A1Forwards(i), A1Discounts(i), A1CallMoneyness{i}, A1TTM(i), cal, Params, alpha, 1);
        A1MdlPricesPut      = @(cal) CallPricesLinearSatoFFT2(A1Forwards(i), A1Discounts(i), A1PutMoneyness{i}, A1TTM(i), cal, Params, alpha, 1) - A1Discounts(i).*(A1Forwards(i) - A1PutStrikes{i})'; 
        ...
        A1MdlPricesCallTemp = @(cal) CallPricesLinearSatoFFT2(A1Forwards(i), A1Discounts(i), A1CallMoneyness{i}, A1TTM(i), cal, Params, alpha, 1);
        A1MdlPricesPutTemp  = @(cal) CallPricesLinearSatoFFT2(A1Forwards(i), A1Discounts(i), A1PutMoneyness{i}, A1TTM(i), cal, Params, alpha, 1) - A1Discounts(i).*(A1Forwards(i) - A1PutStrikes{i})'; 
...
for i = 1:length(A2TTM)
    ...
    if (i==1)
        ...
        A2MdlPricesCall     = @(cal) CallPricesLinearSatoFFT2(A2Forwards(i), A2Discounts(i), A2CallMoneyness{i}, A2TTM(i), cal, Params, alpha, 2);
        A2MdlPricesPut      = @(cal) CallPricesLinearSatoFFT2(A2Forwards(i), A2Discounts(i), A2PutMoneyness{i}, A2TTM(i), cal, Params, alpha, 2) - A2Discounts(i).*(A2Forwards(i) - A2PutStrikes{i})'; 
        ...
        A2MdlPricesCallTemp = @(cal) CallPricesLinearSatoFFT2(A2Forwards(i), A2Discounts(i), A2CallMoneyness{i}, A2TTM(i), cal, Params, alpha, 2);
        A2MdlPricesPutTemp  = @(cal) CallPricesLinearSatoFFT2(A2Forwards(i), A2Discounts(i), A2PutMoneyness{i}, A2TTM(i), cal, Params, alpha, 2) - A2Discounts(i).*(A2Forwards(i) - A2PutStrikes{i})'; 
...
x00  = [0.536616551383301, 0.139734182369473, 26.6886486585019, 2.36333913002023, ...
    0.838756215034617, 0.150662606983024, 28.0274648434157, 2.48189427749875, 0.671563411832399]';
% LB1  = [0, -120, 0.00001]';
LB  = [0.1, 0, 0, 0.1, 0.1, 0, 0, 0.1, 0.5]';
UB  = [120, 120, 100, 100, 120, 120, 100, 100, 1.2]';
% LB2  = [0, -120, 0.00001]';
...
A1C1      = calibparams(1);
A1C2      = calibparams(2);
A1lambda1 = calibparams(3);
A1lambda2 = calibparams(4);

A2C1      = calibparams(5);
A2C2      = calibparams(6);
A2lambda1 = calibparams(7);
A2lambda2 = calibparams(8);

lambda2_z = A1lambda2/A1lambda1;
...
for i = 1:length(A1TTM)
    ...
    A1CalPricesCall    = real(CallPricesLinearSatoFFT2(A1Forwards(i), A1Discounts(i), A1CallMoneyness{i}, A1TTM(i), calibparams, Params, alpha, 1));
    A1CalPricesPut     = real(CallPricesLinearSatoFFT2(A1Forwards(i), A1Discounts(i), A1PutMoneyness{i}, A1TTM(i), calibparams, Params, alpha, 1)) - A1Discounts(i).*(A1Forwards(i) - A1PutStrikes{i})';
    A1mktvolsCall      = blkimpv(A1Forwards(i), A1CallStrikes{i}, A1rates(i), A1TTM(i), A1CallPrices{i});
    ...
    legend
    ylim([0.05 0.3])
    titleName = strcat({'Linear D-IG Sato Smile for'}, {' '}, AssetNames{1,1}, {' '}, {'at'}, {' '}, {datestr(A1Maturities(i))});
...
for i = 1:length(A2TTM)
    A2CalPricesCall    = real(CallPricesLinearSatoFFT2(A2Forwards(i), A2Discounts(i), A2CallMoneyness{i}, A2TTM(i), calibparams, Params, alpha, 2));
    A2CalPricesPut     = real(CallPricesLinearSatoFFT2(A2Forwards(i), A2Discounts(i), A2PutMoneyness{i}, A2TTM(i), calibparams, Params, alpha, 2)) - A2Discounts(i).*(A2Forwards(i) - A2PutStrikes{i})';
    A2mktvolsCall      = blkimpv(A2Forwards(i), A2CallStrikes{i}, A2rates(i), A2TTM(i), A2CallPrices{i});
    ...
    ax.YAxis.FontName = FntNm;
    legend
    titleName = strcat({'Linear D-IG Sato Smile for '}, AssetNames{2,1}, {' '}, {'at'}, {' '}, {datestr(A2Maturities(i))});    
    title(titleName, 'FontSize', FntSz, 'FontName', FntNm)
    xlabel('Moneyness', 'FontSize', FntSz, 'FontName', FntNm)
    ylabel('Implied Volatility', 'FontSize', FntSz, 'FontName', FntNm)
    grid on
...
%% Output on Marginals
MarginalParams = [A1C1, A1C2, A1lambda1, A1lambda2; A2C1, A2C2, A2lambda1, A2lambda2];
CalibError = struct;
...
%% Common parameters
CorrHat = @(cal) (cal(1)./(A1lambda1*A2lambda1) + cal(2)./(A1lambda2*A2lambda2)*sqrt(A1lambda2/A1lambda1))...
    ./(sqrt((A1C1/sqrt(A1lambda1.^3) + A1C2/sqrt(A1lambda2.^3)).*(A2C1/sqrt(A2lambda1.^3) + A2C2/sqrt(A2lambda2.^3))));
d = @(cal) abs(CorrHat(cal) - Correlation);
LB      = [0, 0];
UB      = [min(sqrt(A1lambda1)*A1C1, sqrt(A2lambda1)*A2C1), ...
    min(sqrt(A1lambda2/lambda2_z)*A1C2, sqrt(A2lambda2/lambda2_z)*A2C2)];
[C_z] = fmincon(@(cal) d(cal), [0.1, 0.1], [], [], [], [], LB, UB, [], options);
C1_z = C_z(1,1);
C2_z = C_z(1,2);

A1C1_1 = A1C1 - sqrt(1/A1lambda1)*C1_z;
A2C1_1 = A2C1 - sqrt(1/A2lambda1)*C1_z;

A1C2_2 = A1C2 - sqrt(lambda2_z/A1lambda2)*C2_z;
A2C2_2 = A2C2 - sqrt(lambda2_z/A2lambda2)*C2_z;

a1 = 1/A1lambda1;
a2 = 1/A2lambda1;

CommonParams = [C1_z, C2_z, lambda2_z, calibparams(end)];

CalibCorr = CorrHat(C_z);

MarginalParams = [MarginalParams, [A1C1_1, A1C2_2; A2C1_1, A2C2_2],...
    [a1; a2]];
