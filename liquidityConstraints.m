...
% Only strikes with both Call and Put quotes are considered
%
% INPUT
...
if optionTable.STRIKES(1) ~= optionTable.STRIKES(2)
    optionTableCleaned  = optionTable(2:end, :);
end

if optionTable.STRIKES(end) ~= optionTable.STRIKES(end-1)
    optionTableCleaned  = optionTableCleaned(1:end-1, :);
end
