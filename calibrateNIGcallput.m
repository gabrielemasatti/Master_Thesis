function [MarginalParams, CalibError, A1nOptperMat, A2nOptperMat] = calibrateNIGcallput(SetDate, ...
            A1Discounts, A2Discounts, A1Forwards, A2Forwards, A1DataTab, A2DataTab, ...
                alpha, x00, AssetNames)
%
...
for i = 1:length(A1TTM)
    ...
    if (i==1)
        ...
        A1MdlPricesCall     = @(cal) CallPricesNMVMFFTJoint(A1Forwards(i), A1Discounts(i), A1CallMoneyness{i}, A1TTM(i), cal, Params, alpha, 1);
        A1MdlPricesPut      = @(cal) CallPricesNMVMFFTJoint(A1Forwards(i), A1Discounts(i), A1PutMoneyness{i}, A1TTM(i), cal, Params, alpha, 1) - A1Discounts(i).*(A1Forwards(i) - A1PutStrikes{i})'; 
        ...
        A1MdlPricesCallTemp = @(cal) CallPricesNMVMFFTJoint(A1Forwards(i), A1Discounts(i), A1CallMoneyness{i}, A1TTM(i), cal, Params, alpha, 1);
        A1MdlPricesPutTemp  = @(cal) CallPricesNMVMFFTJoint(A1Forwards(i), A1Discounts(i), A1PutMoneyness{i}, A1TTM(i), cal, Params, alpha, 1) - A1Discounts(i).*(A1Forwards(i) - A1PutStrikes{i})'; 
...
for i = 1:length(A2TTM)
    ...
    if (i==1)
        ...
        A2MdlPricesCall     = @(cal) CallPricesNMVMFFTJoint(A2Forwards(i), A2Discounts(i), A2CallMoneyness{i}, A2TTM(i), cal, Params, alpha, 2);
        A2MdlPricesPut      = @(cal) CallPricesNMVMFFTJoint(A2Forwards(i), A2Discounts(i), A2PutMoneyness{i}, A2TTM(i), cal, Params, alpha, 2) - A2Discounts(i).*(A2Forwards(i) - A2PutStrikes{i})'; 
        ...
        A2MdlPricesCallTemp = @(cal) CallPricesNMVMFFTJoint(A2Forwards(i), A2Discounts(i), A2CallMoneyness{i}, A2TTM(i), cal, Params, alpha, 2);
        A2MdlPricesPutTemp  = @(cal) CallPricesNMVMFFTJoint(A2Forwards(i), A2Discounts(i), A2PutMoneyness{i}, A2TTM(i), cal, Params, alpha, 2) - A2Discounts(i).*(A2Forwards(i) - A2PutStrikes{i})'; 
...
function [c,ceq] = constraints (cal)
  w1  = 1/(2*cal(3).*cal(1).^2);
  w2  = 1/(2*cal(6).*cal(4).^2);  
  c   = [-w1-cal(2), -w2 - cal(4)...
      -cal(1), -cal(4)];
  ceq = [];
end
...
for i = 1:length(A1TTM)
    ...
    A1CalPricesCall    = real(CallPricesNMVMFFTJoint(A1Forwards(i), A1Discounts(i), A1CallMoneyness{i}, A1TTM(i), calibparams, Params, alpha, 1));
    A1CalPricesPut     = real(CallPricesNMVMFFTJoint(A1Forwards(i), A1Discounts(i), A1PutMoneyness{i}, A1TTM(i), calibparams, Params, alpha, 1)) - A1Discounts(i).*(A1Forwards(i) - A1PutStrikes{i})';
    A1mktvolsCall      = blkimpv(A1Forwards(i), A1CallStrikes{i}, A1rates(i), A1TTM(i), A1CallPrices{i});
    ...
    legend
    ylim([0.05 0.3])
    titleName = strcat({'NIG Smile for'}, {' '}, AssetNames{1,1}, {' '}, {'at'}, {' '}, {datestr(A1Maturities(i))});
...
for i = 1:length(A2TTM)
    A2CalPricesCall    = real(CallPricesNMVMFFTJoint(A2Forwards(i), A2Discounts(i), A2CallMoneyness{i}, A2TTM(i), calibparams, Params, alpha, 2));
    A2CalPricesPut     = real(CallPricesNMVMFFTJoint(A2Forwards(i), A2Discounts(i), A2PutMoneyness{i}, A2TTM(i), calibparams, Params, alpha, 2)) - A2Discounts(i).*(A2Forwards(i) - A2PutStrikes{i})';
    A2mktvolsCall      = blkimpv(A2Forwards(i), A2CallStrikes{i}, A2rates(i), A2TTM(i), A2CallPrices{i});
    ...
    ax.YAxis.FontName = FntNm;
    legend
    titleName = strcat({'NIG Smile for '}, AssetNames{2,1}, {' '}, {'at'}, {' '}, {datestr(A2Maturities(i))});    
    title(titleName, 'FontSize', FntSz, 'FontName', FntNm)
    xlabel('Moneyness', 'FontSize', FntSz, 'FontName', FntNm)
    ylabel('Implied Volatility', 'FontSize', FntSz, 'FontName', FntNm)
    grid on
